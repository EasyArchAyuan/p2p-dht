...
import com.ayuan.utils.BTUtil;
import com.ayuan.utils.CodeUtil;
...
public class Node {
    ...
    public static byte[] toBytes(List<Node> nodes) {
        ...
        byte[] result = new byte[nodes.size() * BaseConstant.NODE_BYTES_LEN];
        for (int i = 0; i + BaseConstant.NODE_BYTES_LEN <= result.length; i += BaseConstant.NODE_BYTES_LEN) {
            System.arraycopy(nodes.get(i / BaseConstant.NODE_BYTES_LEN).toBytes(), 0, result, i, BaseConstant.NODE_BYTES_LEN);
        }
        return result;
    }
    ...
    public byte[] toBytes() {
        check();
        //nodeIds
        byte[] nodeBytes = new byte[BaseConstant.NODE_BYTES_LEN];
    ...
    public Node(byte[] bytes) {
        if (bytes.length != BaseConstant.NODE_BYTES_LEN) {
        ...
        //ports
        port = CodeUtil.bytes2Port(ArrayUtils.subarray(bytes, 24, BaseConstant.NODE_BYTES_LEN));
        initHexStrNodeId();
    }
