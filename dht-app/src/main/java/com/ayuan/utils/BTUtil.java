...
import com.ayuan.exception.BTException;
import com.ayuan.pojo.baseenum.MessageTypeEnum;
import com.ayuan.pojo.baseenum.MethodEnum;
import com.ayuan.pojo.entitys.base.BaseConstant;
import com.ayuan.pojo.entitys.peer.Node;
import com.ayuan.process.dto.MessageInfo;
...
public class BTUtil {
     ...
     */
    public static MessageInfo getMessageInfo(Map<String, Object> map) {
        MessageInfo messageInfo = new MessageInfo();
        // 状态 请求/回复/异常
        String y = getParamString(map, "y", "y属性不存在.map:" + map);
        Optional<MessageTypeEnum> yEnumOptional = EnumUtil.getByCode(y, MessageTypeEnum.class);
         ...
         * 如果是请求, 直接从请求主体获取其方法
         */
        if (messageInfo.getStatus().getCode().equals(MessageTypeEnum.QUERY.getCode())) {
        ...
        } else if (EnumUtil.equals(messageInfo.getStatus().getCode(), MessageTypeEnum.RESPONSE)) {
    ...
    public static List<Node> getNodeListByRMap(Map<String, Object> rMap) {
        ...
        for (int i = 0; i + BaseConstant.NODE_BYTES_LEN < nodesBytes.length; i += BaseConstant.NODE_BYTES_LEN) {
            //byte[26] 转 Node
            Node node = new Node(ArrayUtils.subarray(nodesBytes, i, i + BaseConstant.NODE_BYTES_LEN));
            nodeList.add(node);
        }
        return nodeList;
    }
